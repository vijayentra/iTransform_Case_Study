...
function WHistory() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8086/washer/viewWasher/${phoneNumber}`).then((response) => {
        ...
        useEffect(() => {
            const history = async () => {
                try {
                    const response = await axios.get("http://localhost:8082/booking/viewWasherHistory/" + phoneNumber);
    ...
    return (
        <div>
            ...
            <a class="text-grey-500 inline-flex items-center md:mb-2 lg:mb-0 mt-2 ml-2 underline" href="/washerdetails">Back to ALL EMPLOYEES</a>
                <h1 class="text-3xl font-bold text-center text-black-700 py-4 border-b-4 border-grey-700">Employee - {name}/{phoneNumber} - {rating} &#9733;</h1>
                {History.length === 0 ? (
